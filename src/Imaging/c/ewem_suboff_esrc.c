...
/*< extrapolate 1 frequency >*/
{
	...
	ux_xs = alloc1complex(nmx);
	uz_xs = alloc1complex(nmx);
	up_xs = alloc1complex(nmx);
	us_xs = alloc1complex(nmx);
	smig_p = alloc2complex(nz,nmx);
	smig_s = alloc2complex(nz,nmx);

	for (ix=0;ix<nmx;ix++) ux_xg[ix] = 0.;
	for (ix=0;ix<nmx;ix++) uz_xg[ix] = 0.;
	for (ix=0;ix<nmx;ix++) up_xg[ix] = 0.;
	for (ix=0;ix<nmx;ix++) us_xg[ix] = 0.;
	for (ix=0;ix<nmx;ix++) ux_xs[ix] = 0.;
	for (ix=0;ix<nmx;ix++) uz_xs[ix] = 0.;
	for (ix=0;ix<nmx;ix++) up_xs[ix] = 0.;
	for (ix=0;ix<nmx;ix++) us_xs[ix] = 0.;
	if (iw==0) factor = 1.;
	else factor = 2.;
	...
	for (iz=0;iz<nz;iz++){ // extrapolate source wavefield 
		...
		if (z >= sz){
			...
			if (pspi){ 
				...
				ssop(up_xs,w,dkx,nkx,nmx,omx,dmx,dz,iz,vp,po_p,pd_p,p1,p2,true,true,verbose,kz_eps);
				ssop(us_xs,w,dkx,nkx,nmx,omx,dmx,dz,iz,vs,po_s,pd_s,p1,p2,true,true,verbose,kz_eps);
			}
			elastic_separate_2d(ux_xs,uz_xs,up_xs,us_xs,w,dkx,nkx,nmx,omx,dmx,1./po_p[iz],1./po_s[iz],p1,p2,false,false);
			for (ix=0;ix<nmx;ix++) smig_p[ix][iz]  = up_xs[ix]/max_source;
			for (ix=0;ix<nmx;ix++) smig_s[ix][iz]  = us_xs[ix]/max_source;
		}
		else{
			for (ix=0;ix<nmx;ix++) smig_p[ix][iz] = 0.;
			for (ix=0;ix<nmx;ix++) smig_s[ix][iz] = 0.;
		}
	}
	if (adj){
	...
	if (sep){ /* separation of wavefield components to wavefield potentials */
		...
		for (ikx=0;ikx<nkx;ikx++){
			...
			if (bb2 > 0.0000000001f){
				...
				up_k[ikx] = 0.0f;//uz_k[ikx];  
				us_k[ikx] = 0.0f;//ux_k[ikx];
			}
		}
		for (ik=0;ik<nkx;ik++) b[ik] = up_k[ik];
		fftwf_execute_dft(p2,b,b);
		for(imx=0; imx<nmx; imx++) up[imx] = b[imx]/sqrtf(nkx);
		for (ik=0;ik<nkx;ik++) b[ik] = us_k[ik];
		fftwf_execute_dft(p2,b,b);
		for(imx=0; imx<nmx; imx++)us[imx] = b[imx]/sqrtf(nkx);
	}
...
//fprintf(stderr,"kzp*kzs = %f, bb2=%f \n",kzp*kzs,bb2);
			...
			if (bb2 > 0.0000000001f){
				...
				ux_k[ikx] = 0.0f;//us_k[ikx];
				uz_k[ikx] = 0.0f;//up_k[ikx];  
			}
		}
		for (ik=0;ik<nkx;ik++) b[ik] = ux_k[ik];
		fftwf_execute_dft(p2,b,b);
		for(imx=0; imx<nmx; imx++) ux[imx] = b[imx]/sqrtf(nkx);
		for (ik=0;ik<nkx;ik++) b[ik] = uz_k[ik];
		fftwf_execute_dft(p2,b,b);
		for(imx=0; imx<nmx; imx++) uz[imx] = b[imx]/sqrtf(nkx);
	}
