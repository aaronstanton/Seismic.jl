...
void illum(float **m, float **wav,
		...
		int padt, int padx,
		bool pspi, bool verbose,
		float kz_eps)
/*< wave equation source illumination calculation. >*/
{
	...
	progress = 0.;
#pragma omp parallel for private(iw) shared(m_threads,d_s_wx)
	for (iw=ifmin;iw<ifmax;iw++){ 
		progress += 1./((float) ifmax - ifmin);
		if (verbose) progress_msg(progress);
		extrap1f(m_threads,d_s_wx,iw,ifmax,nw,ifmax,ntfft,dw,dkx,dky,nkx,nky,nz,oz,dz,sz,nmx,omx,dmx,nmy,omy,dmy,nthread,vel,po,pd,vref,iref1,iref2,nref,p1,p2,pspi,verbose,kz_eps);
	}
...
void extrap1f(float **m,complex **d_s_wx,
...
/*< extrapolate 1 frequency >*/
{
	...
	w = iw*dw;
		...
		for (iz=0;iz<nz;iz++){ // extrapolate source wavefield
			...
			for (imx=0;imx<nmx;imx++){ 
				for (imy=0;imy<nmy;imy++){
					m[imx*nmy*nthread + imy*nthread + ithread][iz] += factor*cabsf(d_xs[imx*nmy + imy]);
				}
			}
		}
